...
def partition_graph(graph: dict[str, list[str]]) -> set[tuple[str, str]]:
    ...
    delete_edges = [(node, neighbor)
        for node in groups[0]
        for neighbor in graph[node]
        if neighbor in groups[1]]
    return groups, len(delete_edges)
    # return {
    #     (node, neighbor)
    #     for node in groups[0]
    #     for neighbor in graph[node]
...
def get_best_partition(graph, num=100):
    TEST_GRAPH = {
        "1": ["2", "3", "4", "5"],
        "2": ["1", "3", "4", "5"],
        "3": ["1", "2", "4", "5", "10"],
    ...
    best_group_bias = -1
    for i in range(num):
        groups, del_edge_num = partition_graph(graph)
        group_bias = abs(len(groups[0]) - len(groups[1]))
        if best_del_edge_num == -1 or del_edge_num < best_del_edge_num\
                or (del_edge_num == best_del_edge_num and group_bias < best_group_bias):
            best_groups = groups
            best_del_edge_num = del_edge_num
            best_group_bias = group_bias
    return best_groups, best_del_edge_num
