...
function evaluatePalette(palette) {
    if (color_names_checked != undefined && color_names_checked.length > 0) {
        ...
        if (count > global_count) // TODO if there are more than two colors that are not in selected color names, then discard this palette
            return -1;
    }
...
function isDiscriminative2(pos_colors, fixed_colors) {
    ...
    for (let i = 0; i < fixed_colors.length; i++) {
        for (let j = 0; j < pos_colors.length; j++) {
            let color_dis = d3_ciede2000(d3mlab(fixed_colors[i][0]), d3mlab(pos_colors[j][0]));
            if (color_dis < global_color_dis) {
    ...
    for (let j = 0; j < pos_colors.length; j++)
        for (let k = j + 1; k < pos_colors.length; k++) {
            color_dis = d3_ciede2000(d3mlab(pos_colors[j][0]), d3mlab(pos_colors[k][0]));
            if (color_dis < global_color_dis) {
