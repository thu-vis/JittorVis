...
class PosEmbedRel(nn.Module):
    ...
    def __init__(self, feat_size, dim_head, scale):
        ...
        self.scale = scale
        self.height_rel = jt.randn(self.height * 2 - 1, dim_head) * self.scale
        self.width_rel = jt.randn(self.width * 2 - 1, dim_head) * self.scale
...
class BottleneckAttn(nn.Module):
    ...
    def execute(self, x):
        ...
        x = x.reshape(B, -1, self.dim_head, H * W).transpose(0, 1, 3, 2)
        q, k, v = jt.split(x, self.num_heads, dim=1)

        attn_logits = (q @ k.transpose(0, 2, 1)) * self.scale
        attn_logits = attn_logits + self.pos_embed(q)  # B, num_heads, H * W, H * W

        attn_out = attn_logits.softmax(dim=-1)
        attn_out = (attn_out @ v).transpose(0, 2, 1).reshape(B, self.dim_out, H, W)  # B, dim_out, H, W
