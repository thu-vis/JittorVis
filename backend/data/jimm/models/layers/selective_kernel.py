...
class SelectiveKernelAttn(nn.Module):
    def __init__(self, channels, num_paths=2, attn_channels=32,
                 act_layer=nn.ReLU, norm_layer=BatchNorm2d):
...
class SelectiveKernel(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, kernel_size=None, stride=1, dilation=1, groups=1,
                 ...
                 drop_block=None, act_layer=nn.ReLU, norm_layer=BatchNorm2d, aa_layer=None):
    ...
    def execute(self, x):
        if self.split_input:
            x_split = jt.split(x, self.in_channels // self.num_paths, 1)
            x_paths = [op(x_split[i]) for i, op in enumerate(self.paths)]
        else:
            x_paths = [op(x) for op in self.paths]
        x = jt.stack(x_paths, dim=1)
        x_attn = self.attn(x)
        x = x * x_attn
        x = x.sum(dim=1)
        return x
