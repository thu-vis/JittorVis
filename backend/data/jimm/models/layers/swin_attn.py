...
def window_partition(x, win_size: int):
    ...
    windows = x.permute(0, 1, 3, 2, 4, 5).view(-1, win_size, win_size, C)
    return windows
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            ...
            H, W = self.feat_size
            img_mask = jt.zeros((1, H, W, 1))  # 1 H W 1
            h_slices = (
        ...
        self.relative_position_bias_table = jt.zeros((2 * self.win_size - 1) * (2 * self.win_size - 1), num_heads)
        # 2 * Wh - 1 * 2 * Ww - 1, nH

        trunc_normal_(self.relative_position_bias_table, std=.02)

        # get pair-wise relative position index for each token inside the window
        coords_h = jt.arange(self.win_size)
        coords_w = jt.arange(self.win_size)
    ...
    def execute(self, x):
        ...
        relative_position_bias = self.relative_position_bias_table[
            self._relative_position_index.view(-1)].view(win_size_sq, win_size_sq, -1)
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh * Ww, Wh * Ww
        attn = attn + relative_position_bias.unsqueeze(0)
        if self._attn_mask is not None:
            num_win = self._attn_mask.shape[0]
            attn = attn.view(B, num_win, self.num_heads, N, N) + self._attn_mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(0, 2, 1, 3).reshape(BW, N, self.dim_out)
