...
class GatherExcite(nn.Module):
    ...
    def __init__(
            ...
            act_layer=nn.ReLU, norm_layer=BatchNorm2d, gate_layer='sigmoid'):
        ...
        if extra_params:
            ...
            if extent == 0:
                ...
                if norm_layer:
                    self.gather.add_module(f'norm1', BatchNorm2d(channels))
            else:
                assert extent % 2 == 0
                ...
                for i in range(num_conv):
                    ...
                    if norm_layer:
                        self.gather.add_module(f'norm{i + 1}', BatchNorm2d(channels))
                    if i != num_conv - 1:
                        self.gather.add_module(f'act{i + 1}', act_layer())
        else:
            self.gather = None
            if self.extent == 0:
                self.gk = 0
