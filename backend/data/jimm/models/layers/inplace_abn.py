...
class InplaceAbn(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, apply_act=True,
        ...
        self.running_mean = jt.zeros(num_features).stop_grad()
        self.running_var = jt.ones(num_features).stop_grad()
    ...
    def execute(self, x):
        output = inplace_abn(
            ...
            self.is_training(), self.momentum, self.eps, self.act_name, self.act_param)
