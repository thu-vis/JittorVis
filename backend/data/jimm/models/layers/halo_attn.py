...
def rel_logits_1d(q, rel_k, permute_mask: List[int]):
    ...
    l = len(rel_k.shape)
    x = (q @ rel_k.transpose(*range(l)[:-2], l - 1, l - 2))
...
class PosEmbedRel(nn.Module):
    ...
    def __init__(self, block_size, win_size, dim_head, scale):
        ...
        self.scale = scale
        self.height_rel = jt.randn(win_size * 2 - 1, dim_head) * self.scale
        self.width_rel = jt.randn(win_size * 2 - 1, dim_head) * self.scale
...
class HaloAttn(nn.Module):
    ...
    def execute(self, x):
        ...
        q = q.reshape(B * self.num_heads, self.dim_head, -1, num_blocks).transpose(0, 3, 2, 1)
        ...
        kv = kv.reshape(
            B * self.num_heads, self.dim_head + (self.dim_v // self.num_heads), -1, num_blocks).transpose(0, 3, 2, 1)
        k, v = jt.split(kv, [self.dim_head, self.dim_v // self.num_heads], dim=-1)

        attn_logits = (q @ k.transpose(0, 1, 3, 2)) * self.scale  # FIXME should usual attn scale be applied?
        attn_logits = attn_logits + self.pos_embed(q)  # B * num_heads, block_size ** 2, win_size ** 2

        attn_out = attn_logits.softmax(dim=-1)
        attn_out = (attn_out @ v).transpose(0, 3, 2,
                                            1)  # B * num_heads, dim_v // num_heads, block_size ** 2, num_blocks
        attn_out = F.fold(
