...
class PreActBottleneck(nn.Module):
    ...
    def zero_init_last_bn(self):
        nn.init.constant_(self.norm3.weight, 0.0)
...
class ResNetV2(nn.Module):
    ...
    def __init__(
        ...
        curr_stride = 4
        dilation = 1
        block_dprs = [x.tolist() for x in jt.linspace(0, drop_path_rate, sum(layers)).split(layers, dim=0)]
...
def _init_weights(module: nn.Module, name: str = '', zero_init_last_bn=True):
    if isinstance(module, nn.Linear) or ('head.fc' in name and isinstance(module, nn.Conv2d)):
        nn.init.gauss_(module.weight, mean=0.0, std=0.01)
...
def _load_weights(model: nn.Module, checkpoint_path: str, prefix: str = 'resnet/'):
    ...
    for i, (sname, stage) in enumerate(named_children(model.stages)):
        for j, (bname, block) in enumerate(named_children(stage.blocks)):
