...
from .layers import create_classifier, BatchNorm2d
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        if stride != 1 or self.num_inchannels[branch_index] != num_channels[branch_index] * block.expansion:
            downsample = nn.Sequential(
                ...
                BatchNorm2d(num_channels[branch_index] * block.expansion, momentum=_BN_MOMENTUM),
            )
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(nn.Sequential(
                        ...
                        BatchNorm2d(num_inchannels[i], momentum=_BN_MOMENTUM),
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(nn.Sequential(
                                ...
                                BatchNorm2d(num_outchannels_conv3x3, momentum=_BN_MOMENTUM)))
                        else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, global_pool='avg', drop_rate=0.0, head='classification'):
        ...
        self.bn1 = BatchNorm2d(stem_width, momentum=_BN_MOMENTUM)
        self.act1 = nn.ReLU()
        self.conv2 = nn.Conv2d(stem_width, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn2 = BatchNorm2d(64, momentum=_BN_MOMENTUM)
        self.act2 = nn.ReLU()
    ...
    def _make_head(self, pre_stage_channels, incre_only=False):
        ...
        for i in range(len(pre_stage_channels) - 1):
            ...
            downsamp_module = nn.Sequential(
                ...
                BatchNorm2d(out_channels, momentum=_BN_MOMENTUM),
                nn.ReLU()
            )
        ...
        final_layer = nn.Sequential(
            ...
            BatchNorm2d(self.num_features, momentum=_BN_MOMENTUM),
            nn.ReLU()
        )
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        ...
                        BatchNorm2d(num_channels_cur_layer[i], momentum=_BN_MOMENTUM),
                        nn.ReLU()))
                else:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(nn.Sequential(
                        ...
                        BatchNorm2d(outchannels, momentum=_BN_MOMENTUM),
                        nn.ReLU()))
