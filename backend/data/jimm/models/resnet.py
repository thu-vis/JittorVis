...
from .layers import DropBlock2d, DropPath, AvgPool2dSame, BlurPool2d, create_attn, create_classifier, BatchNorm2d
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=BatchNorm2d,
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
                 ...
                 act_layer=nn.ReLU, norm_layer=BatchNorm2d, aa_layer=None, drop_rate=0.0, drop_path_rate=0.,
    ...
    def execute(self, x):
        ...
        if self.drop_rate:
            x = F.dropout(x, p=float(self.drop_rate), is_train=self.is_train)
        x = self.fc(x)
        return x
