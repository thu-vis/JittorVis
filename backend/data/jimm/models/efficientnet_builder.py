    ...
    * https://github.com/tensorflow/tpu/blob/master/models/official/efficientnet/efficientnet_model.py
    ...
    if isinstance(m, CondConv2d):
        ...
        init_weight_fn = get_condconv_initializer(
            lambda w: init.gauss_(w, 0, math.sqrt(2.0 / fan_out)), m.num_experts, m.weight_shape)
        ...
        if fix_group_fanout:
            fan_out //= m.groups
        # m.weight.normal(0, math.sqrt(2.0 / fan_out))
        ...
        # m.bias.data.zero_()
        init.uniform_(m.weight, -init_range, init_range)
        init.constant_(m.bias, 0.0)
