...
from .layers import create_conv2d, drop_path, make_divisible, create_act_layer, BatchNorm2d,adaptive_avg_pool2d
...
class SqueezeExcite(nn.Module):
    ...
    def __init__(
        ...
        self.act1 = create_act_layer(act_layer)
        self.conv_expand = nn.Conv2d(rd_channels, in_chs, 1, bias=True)
        self.gate = create_act_layer(gate_layer)

    def execute(self, x):
        x_se = x.mean((2, 3)).unsqueeze(-1).unsqueeze(-1)
...
class ConvBnAct(nn.Module):
    ...
    def __init__(
            ...
            skip=False, act_layer=nn.ReLU, norm_layer=BatchNorm2d, drop_path_rate=0.):
    ...
    def execute(self, x):
        ...
        if self.has_residual:
            if self.drop_path_rate > 0.:
                x = drop_path(x, self.drop_path_rate, self.is_training())
            x += shortcut
        return x
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def __init__(
            ...
            noskip=False, pw_kernel_size=1, pw_act=False, act_layer=nn.ReLU, norm_layer=BatchNorm2d,
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=BatchNorm2d, se_layer=None, conv_kwargs=None, drop_path_rate=0.):
...
class CondConvResidual(InvertedResidual):
    ...
    def __init__(
            self, in_chs, out_chs, dw_kernel_size=3, stride=1, dilation=1, pad_type='',
            noskip=False, exp_ratio=1.0, exp_kernel_size=1, pw_kernel_size=1, act_layer=nn.ReLU,
            norm_layer=BatchNorm2d, se_layer=None, num_experts=0, drop_path_rate=0.):
    ...
    def execute(self, x):
        ...
        # CondConv routing
        pooled_inputs = adaptive_avg_pool2d(x, 1).flatten(1)
        routing_weights = self.routing_fn(pooled_inputs).sigmoid()
...
class EdgeResidual(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=BatchNorm2d, se_layer=None, drop_path_rate=0.):
