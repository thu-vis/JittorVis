...
from .layers import DropPath, to_2tuple, trunc_normal_,BatchNorm2d
import math
import numpy as np
...
class OutlookAttention(nn.Module):
    ...
    def execute(self, x):
        ...
        v = F.unfold(v, kernel_size=self.kernel_size,
                     padding=self.padding,
                     stride=self.stride).reshape(B, self.num_heads,
                                                 C // self.num_heads,
...
class ClassBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        return jt.concat([cls_embed, x[:, 1:]], dim=1)
...
class VOLO(nn.Module):
    ...
    def __init__(self, layers, img_size=224, in_chans=3, num_classes=1000, patch_size=8,
        ...
        self.pos_embed = jt.zeros((1, img_size // patch_size // pooling_scale,
        ...
        if post_layers is not None:
            ...
            self.cls_token = jt.zeros((1, 1, embed_dims[-1]))
    ...
    def execute(self, x):
        ...
        if self.mix_token and self.is_training():
        ...
        if self.mix_token and self.is_training():  # reverse "mix token", see token labeling for details.
